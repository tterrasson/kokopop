...
public:
    ...
    // Metal's scheduler splices in more copy nodes than the others (every
    // CPU-pinned op becomes a pair of transfers), and the frontend graph used
    // to be given 64 MiB for exactly that reason. Keep that margin.
    size_t graph_context_bytes(size_t n_tensors, size_t n_nodes) const override {
        return graph_context_bytes_with_margin(n_tensors, n_nodes, backend_mib(64));
    }
};
